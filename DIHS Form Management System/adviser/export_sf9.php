...
$tempFile = tempnam(sys_get_temp_dir(), 'sf9_');
if ($tempFile === false) {
...
try {
    ...
    if (!file_exists($tempFile) || filesize($tempFile) === 0) {
        throw new Exception('Failed to generate Excel file: File is empty');
    }
    ...
    header('Content-Disposition: attachment; filename="SF9_' . $lrn . '_' . date('Y-m-d_His') . '.xlsx"');
