...
// Debug function to log queries and results
function debug_query($query, $params = []) {
    $log_file = __DIR__ . '/query_debug.log';
    $timestamp = date('Y-m-d H:i:s');
    $log_entry = "[$timestamp] Query: $query\n";
    
    if (!empty($params)) {
        $log_entry .= "Params: " . print_r($params, true) . "\n";
    }
    
    // Log the backtrace to see where the query is coming from
    ...
    if ($result === false) {
        ...
        file_put_contents($log_file, "[$timestamp] Query Error: $error\n\n", FILE_APPEND);
    } else {
        file_put_contents($log_file, "[$timestamp] Query executed successfully\n\n", FILE_APPEND);
    }
...
set_exception_handler(function($e) {
    ...
    // Log to error file
    file_put_contents(__DIR__ . '/error.log', $error_message, FILE_APPEND);
    ...
    die("An unexpected error occurred. Please check the error log for details.");
});

// Log the start of the script
file_put_contents(__DIR__ . '/export_debug.log', "[" . date('Y-m-d H:i:s') . "] Script started\n", FILE_APPEND);
...
if ($conn) {
    file_put_contents(__DIR__ . '/export_debug.log', "[" . date('Y-m-d H:i:s') . "] Database connection successful\n", FILE_APPEND);
} else {
    file_put_contents(__DIR__ . '/export_debug.log', "[" . date('Y-m-d H:i:s') . "] Database connection failed: " . mysqli_connect_error() . "\n", FILE_APPEND);
    die("Database connection failed");
}

// Log the current database
$db_result = debug_query("SELECT DATABASE() as db");
if ($db_result) {
    $db_name = mysqli_fetch_assoc($db_result)['db'];
    file_put_contents(__DIR__ . '/export_debug.log', "[" . date('Y-m-d H:i:s') . "] Current database: $db_name\n", FILE_APPEND);
}

// Log all tables in the database
$tables_result = debug_query("SHOW TABLES");
if ($tables_result) {
    $tables = [];
    while ($row = mysqli_fetch_row($tables_result)) {
        $tables[] = $row[0];
    }
    file_put_contents(__DIR__ . '/export_debug.log', "[" . date('Y-m-d H:i:s') . "] Available tables: " . implode(', ', $tables) . "\n", FILE_APPEND);
}
