...
try {
    ...
    while (true) {
        ...
        // Handle age
        $ageVal = $worksheet->getCell('J' . $row)->getValue();
        $age = is_numeric($ageVal) ? (int)$ageVal : null;

        $religious_affiliation = (string) $worksheet->getCell('L' . $row)->getValue();
        $house_street_sitio_purok = (string) $worksheet->getCell('M' . $row)->getValue();
        $barangay = (string) $worksheet->getCell('N' . $row)->getValue();
        $municipality_city = (string) $worksheet->getCell('R' . $row)->getValue();
        $province = (string) $worksheet->getCell('U' . $row)->getValue();
        $fathers_name = (string) $worksheet->getCell('W' . $row)->getValue();
        $mothers_maiden_name = (string) $worksheet->getCell('X' . $row)->getValue();
        $name_guardian = (string) $worksheet->getCell('Z' . $row)->getValue();
        $relationship = (string) $worksheet->getCell('AC' . $row)->getValue();
        $contact_number = (string) $worksheet->getCell('AD' . $row)->getValue();
        $remarks = (string) $worksheet->getCell('AE' . $row)->getValue();
